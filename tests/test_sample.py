...
class TestDraw:
    ...
    def test_draws_the_requested_size(self):
        chosen, _, _ = draw(60)
    ...
    def test_every_stratum_is_represented(self):
        ...
        _, allocation, grouped = draw(60)
        assert all(allocation[k] >= 1 for k in grouped)

    def test_never_over_draws_a_small_stratum(self):
        _, allocation, grouped = draw(60)
        assert all(allocation[k] <= len(grouped[k]) for k in grouped)

    def test_not_just_the_first_n(self):
        """A sample of the top of the thread is a sample of whoever posted on the 1st."""
        from ingest.corpus import posts

        chosen, _, _ = draw(30)
        head = {c["objectID"] for c in posts()[:30]}
        assert len({c["objectID"] for c in chosen} & head) < 25


class TestOversample:
    def test_boosts_the_named_slice(self):
        _, base, _ = draw(60)
        _, boosted, grouped = draw(60, oversample={"prose": 18})
    ...
    def test_still_draws_the_requested_size(self):
        chosen, _, _ = draw(60, oversample={"prose": 18})
        assert len(chosen) == 60

    def test_unknown_value_is_a_no_op_not_a_crash(self):
        chosen, _, _ = draw(60, oversample={"nonsense": 20})
        assert len(chosen) == 60


class TestSplit:
    def test_two_thirds_dev(self):
        chosen, _, _ = draw(60)
        counts = split(chosen)
    ...
    def test_every_post_assigned_exactly_once(self):
        chosen, _, _ = draw(60)
...
class TestSlicePower:
    ...
    def test_margins_are_computed_not_hardcoded(self):
        chosen, _, _ = draw(60)
